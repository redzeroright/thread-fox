...
function buildPrompt({ topic, purpose, hook }) {
  ...
  return [
    '너는 한국어 소셜미디어 "스레드(Threads)" 전문 카피라이터야.',
    '아래 조건에 맞춰 서로 확실히 다른 스레드 글을 작성해.',
    '',
    `- 주제: "${topic}"`,
    ...
    '',
    '작성 규칙:',
    `1) 총 ${n}개의 글을 만들어. ${n}개 모두 위 "시작 훅 스타일"로 시작하되, 서로 다른 각도/소재로 접근해 겹치지 않게 해.`,
    '2) 각 글은 위 "글의 목적"을 달성하도록 써.',
    '3) 자연스러운 반말 구어체, 사람이 직접 쓴 느낌. 광고 티·상투적 표현 지양.',
    '4) 각 글은 한국어 기준 대략 120~200자. 줄바꿈(\\n)을 1~2회 넣어 가독성을 살려.',
    '5) 각 글에 상황에 어울리는 이모지를 딱 1개만 자연스럽게 포함.',
    '6) 해시태그·따옴표로 감싼 제목·머리말("[정보]" 등)은 넣지 마.',
    '7) template 필드에는 그 글의 접근 방식을 나타내는 짧은 한국어 라벨을 넣어 (예: "공감형", "정보형", "반전형", "경험담형" 등).',
    '',
