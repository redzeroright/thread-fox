...
function buildDebateRules(hook) {
  const hookFirstSentence = {
    "공감 훅": "첫 문장은 반드시 독자가 겪을 법한 망설임·감정·일상 장면으로 시작한다. 첫 문장에 결론·판정·논쟁 주장을 먼저 쓰지 않는다. 예: ‘자소서 쓰다 GPT에 손이 가는 순간, 한 번쯤 망설여지지 않아?’",
    "의문형 훅": "첫 문장은 독자가 자기 경험을 떠올릴 수 있는 자연스러운 질문으로 시작한다. 답을 유도하는 공격적 질문은 피한다.",
    "숫자 훅": "첫 문장은 꾸며낸 통계 없이, 논쟁을 살펴볼 2~3개의 관점이나 기준을 예고하는 숫자 훅으로 시작한다.",
    "반전 훅": "첫 문장은 흔한 통념을 과장 없이 뒤집는 관찰로 시작한다. 뒤집는 이유는 두 번째 문장에서 설명한다.",
    "도발 훅": "첫 문장은 타인·집단을 공격하지 않는 선에서 선명한 관점을 제시한다. 혐오·조롱·낙인은 금지한다.",
    "스토리 훅": "첫 문장은 사용자가 제공하지 않은 실제 경험을 꾸며내지 않는 일반적 장면으로 시작한다. 예: ‘마감이 가까워질수록 도구에 기대고 싶어지는 순간이 있다.’",
  };

  return [
    "",
    "논쟁유발 모드 추가 규칙:",
    `1) 선택한 훅은 최우선 규칙이다. ${hookFirstSentence[hook] || hookFirstSentence["공감 훅"]}`,
    "2) 두 번째 문장에는 ‘하지만’, ‘그런데’, ‘나는’ 등 자연스러운 전환을 사용해 찬반이 실제로 갈릴 수 있는 하나의 분명한 주장을 제시한다.",
    "3) 이어서 그 주장을 뒷받침하는 구체적인 이유·관찰·일반 원칙을 한 가지 이상 제시한다. 자극적인 단정이나 꾸며낸 통계는 쓰지 않는다.",
    "4) 반대 의견이 나올 수 있는 합리적 이유를 한 번 공정하게 인정한 뒤, 자신의 관점을 다시 명확히 한다.",
    "5) 마지막 문장은 독자가 자신의 경험·근거·반론을 남기고 싶게 만드는 열린 질문으로 끝낸다.",
    "6) 세 초안은 서로 다른 논쟁 축과 서로 다른 공감 장면을 사용한다. 예: 효율 대 진정성, 편의 대 공정성, 단기 성과 대 장기 신뢰.",
    "7) 특정 개인·직업·세대·집단을 비하하거나 공격하지 않고, 확인되지 않은 통계·사실·경험을 만들지 않는다.",
  ];
}

function buildPrompt({ topic, purpose, hook }) {
  const debateRules = purpose === "논쟁유발" ? buildDebateRules(hook) : [];
